...
def assign_grade(z_score, scale="1_to_6"):
    if scale == "1_to_6":
        if z_score <= -2: return 1
        elif -2 < z_score <= -1: return 2
        elif -1 < z_score <= 0: return 3
        elif 0 < z_score <= 1: return 4
        elif 1 < z_score <= 2: return 5
        else: return 6
    elif scale == "0_to_5":
...
df['z_score_grade'] = df['z_score'].apply(lambda x: assign_grade(x, scale="0_to_5"))
df['z_score_1y_grade'] = df['z_score_1y'].apply(lambda x: assign_grade(x, scale="0_to_5"))
df['z_score_3y_grade'] = df['z_score_3y'].apply(lambda x: assign_grade(x, scale="0_to_5"))

# Save the cleaned and processed DataFrame to a CSV file
df.to_csv('cleaned_BRA_FINAL_TEST_scorecard.csv', index=False)